    ...
    >
        ...
        <div class="flex justify-between q-gutter-sm" style="width: 100%">
          <div class="q-mb-sm flex q-gutter-sm">
            <q-btn
              color="blue-grey"
              icon="o_add"
              @click="ui.DatabaseDialog.show = true"
            ></q-btn>

            <q-btn
              :href="ui.PHPMyAdmin.url"
              color="yellow-10"
              icon="o_login"
              target="_blank"
              type="a"
            ...
            <q-btn
              v-if="tableSelected.length === 1"
              color="red"
              icon="o_remove"
              @click="requestDeleteDatabase"
            ></q-btn>
          ...
          <div class="flex justify-end q-gutter-sm items-baseline no-wrap">
            ...
            <div>
              <q-icon
                name="o_settings"
                size="18px"
                @click="toDatabaseSettings(props.key)"
              ></q-icon>
            </div>
          </div>
        </q-td>
      ...
      <template v-slot:body-cell-status="props">
        ...
        <q-td :props="props">
          ...
          <div
            v-else-if="props.row.create_status === 0"
...
export default {
  ...
  setup(props, {emit}) {
    ...
    function requestDeleteDatabase() {
      showLoading($q);
      deleteDatabase(tableSelected.value[0].id)
        .then((res) => {
          requestInstance();
        })
        .catch((err) => {
