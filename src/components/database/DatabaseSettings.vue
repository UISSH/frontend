    ...
    <q-tab-panels v-model="tab" animated>
      <q-tab-panel name="settings">
        <q-card class="apple-card">
          <div
            class="bg-blue-grey-1 flex justify-between q-pa-md items-center content-center rounded-borders"
          >
            <div class="text-body1 text-capitalize">
              {{ ui.database.data.name }} Settings
            </div>
            <div>
              <q-btn
                color="dark"
                flat
                icon="o_refresh"
                @click="requestGetDatabase"
              ></q-btn>
            </div>
          </div>
          <q-card-section>
            <q-input
              v-model="ui.database.data.name"
              color="dark"
              label="database name"
              readonly
            ></q-input>
            <q-input
              v-model="ui.database.data.username"
              color="dark"
              label="username"
            ></q-input>
            <q-input
              v-model="ui.database.data.password"
              :type="ui.database.isPwd ? 'password' : 'text'"
              color="dark"
              label="password"
            >
            ...
            <div class="flex q-gutter-sm">
              ...
              >
                ...
                <template v-slot:after>
                  <q-btn
                    :disable="ui.uploadFile.data == null"
                    flat
                    icon="o_upload"
            ...
            <template v-slot:body-cell-action="props">
              <q-td :props="props">
                <q-btn
                  ...
                  label="recover"
                  no-caps
                  @click="requestImportBackup(props.value)"
                ></q-btn>
                <q-btn
                  color="dark"
                  flat
                  ...
                  label="download"
                  no-caps
                  @click="requestDownloadFile(props.value)"
                ></q-btn>
              </q-td>
            </template>
          </q-table>
...
export default {
  ...
  setup: function (props) {
    ...
    function requestExportBackup() {
      showLoading($q);
      exportBackup(ui.value.database.data.id)
        .then(async (res) => {
    ...
    function requestListDirectory() {
      ...
      listDirectory(dbBackupsDirectory)
        .then((res) => {
          tableData.value = [];
    ...
    function requestUploadFile() {
      showLoading($q);
      let dbBackupsDirectory = `/var/db_backups/${ui.value.database.data.name}`;
      uploadFile(dbBackupsDirectory, ui.value.uploadFile.data)
        .then(async (res) => {
          console.log(res);
          await requestListDirectory();
        })
