  ...
  <div class="flex justify-between">
    <div class="q-gutter-sm">
      <q-btn
        ...
        color="blue"
        icon="add"
        @click="ui.NewWebsiteDialog.show = true"
      ></q-btn>
      <q-btn
        v-if="ui.StartWebsiteBtn.show"
        :label="$t('start')"
        color="green"
        icon="play_arrow"
        @click="requestApplicationAction('start')"
      ></q-btn>
      <q-btn
        v-if="ui.StopWebsiteBtn.show"
        :label="$t('stop')"
        color="grey"
        icon="stop"
        @click="requestApplicationAction('stop')"
      ></q-btn>
      <q-btn
        v-if="ui.DeleteWebsiteBtn.show"
        :label="$t('delete')"
        color="red"
        icon="delete"
        @click="requestDeleteWebsite"
      ></q-btn>
    </div>
    <div>
      <q-btn
  ...
  >
    <template v-slot:header-cell="props">
      <q-th class="text-body1 text-purple">
        <q-td
          :class="`flex ${Public.formatColumnAlignCss(
            props.col.align
          )} no-wrap`"
          :props="props"
        >
    ...
    <template v-slot:body-cell-domain="props">
      <q-td :props="props">
        <div
          class="flex justify-start items-center q-gutter-sm no-wrap"
          style="cursor: pointer"
        >
          <div @click="Public.openDomain(props.row.ssl_enable, props.value)">
            {{ props.value }}
          </div>
          <q-icon
            :color="props.row.ssl_enable ? 'green' : 'blue-grey'"
            name="public"
            @click="Public.openDomain(props.row.ssl_enable, props.value)"
          ></q-icon>
        </div>
      </q-td>
    </template>
    ...
    <template v-slot:body-cell-application="props">
      <q-td :props="props">
        ...
        >
          <div>
            {{ props.value ? props.value.replace("Application", "") : "" }}
          </div>

          <q-icon
            color="blue-grey"
            name="o_settings"
            @click="toWebsiteSettings(props.key)"
          ></q-icon>
        </div>
      </q-td>
    </template>
    ...
    <template v-slot:body-cell-ssl="props">
      <q-td :props="props">
        ...
        >
          <q-toggle
            :model-value="props.row.ssl_enable"
            checkedIcon="enhanced_encryption"
            color="green"
            unchecked-icon="no_encryption"
            @click="requestPutWebsiteSSL(props.row)"
          ></q-toggle>
        </div>
      </q-td>
    </template>
