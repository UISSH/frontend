    ...
    <q-card-section>
      <div class="flex q-gutter-sm justify-around">
        <div class="text-center">
          <q-circular-progress
            :class="`text-${Public.getSystemStatusColor(
              serverStatus.loadAverage.per_core_used
            )} q-ma-md`"
            :color="
              Public.getSystemStatusColor(
                serverStatus.loadAverage.per_core_used
              )
            "
          ...
          >
            ...
            <q-tooltip v-if="serverStatus.loadAverage.info">
              <div
                v-for="item in serverStatus.loadAverage.info"
                :key="item"
                class="text-body2 text-capitalize"
              >
        ...
        <div class="text-center">
          ...
          >
            ...
            <q-tooltip>
              <div
                v-for="(value, item) in serverStatus.cpu.info_1"
                :key="item"
                class="text-body2 text-capitalize"
              >
                {{ item }} :{{ value }}
        ...
        <div class="text-center">
          ...
          >
            ...
            <q-tooltip>
              <div
                v-for="(value, item) in serverStatus.memory.info"
                :key="item"
                class="text-body2 text-capitalize"
              >
          ...
          >
            ...
            <q-tooltip v-if="serverStatus.disk.info">
              <div class="text-body2 text-capitalize">
                total : {{ serverStatus.disk.info.total_space_gb }} GiB
              </div>
              <div class="text-body2 text-capitalize">
                available : {{ serverStatus.disk.info.available }}%
              </div>
              <div class="text-body2 text-capitalize">
                used : {{ serverStatus.disk.info.used }}%
              </div>
              <div class="text-body2 text-capitalize">
                available : {{ serverStatus.disk.info.space_left_gb }} GiB
              </div>
              <div class="text-body2 text-capitalize">
                used: {{ serverStatus.disk.info.used_space_gb }} GiB
              </div>
            </q-tooltip>
          </q-circular-progress>
  ...
  setup() {
    ...
    onBeforeUnmount(() => {
      for (let item of data.value.intervalID) {
        console.debug(
          `onBeforeUnmount clear job [${item.mark}] interval id:${item.id} `
        );
